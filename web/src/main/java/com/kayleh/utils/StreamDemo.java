...
public class StreamDemo {
    public static void main(String[] args) {
        ...
        int compare = comparator.compare(100, 1);
        IntBinaryOperator intBinaryOperator = Integer::compare;
        intBinaryOperator.applyAsInt(10, 100);
        ...
        Stream<String> c = Stream.concat(d, b);

        /**
         * @max
         */
        //一般用于求数字集合中的最大值，或者按实体中数字类型的属性比较，拥有最大值的那个实体。它接收一个 Comparator<T>，
        //上面也举到这个例子了，它是一个函数式接口类型，专门用作定义两个对象之间的比较，例如下面这个方法使用了 Integer::compareTo这个方法引用。
        Stream<Integer> integerStream = Stream.of(2, 2, 100, 5);
        Integer max = integerStream.max(Integer::compareTo).get();
        System.out.println(max);
        //定制一个 Comparator
        Stream<Integer> integerStream1 = Stream.of(2, 2, 100, 5);
        Comparator<Integer> comparator1 = (x, y) -> (x.intValue() < y.intValue()) ? -1 : ((x.equals(y) ? 0 : 1));
        Integer max1 = integerStream1.max(comparator1).get();
        ...
        //顾名思义，它是用来消费 Stream 元素的，比如下面这个方法，把每个元素转换成对应的大写字母并输出。
        ...
        //这个 Stream 就真的被消费掉了，之后这个 Stream 流就没有了，不可以再对它进行后续操作了，而 peek操作完之后，还是一个可操作的 Stream 对象。
        ...
        for (int k = 1; k <= 10; k++) {
            User user = new User();
            user.setUserId(k);
            user.setUserName(String.format("古时的风筝 %s 号", k));
         ...
         * @map
         */
        // Function的声明是这样的，观察 apply方法，接受一个 T 型参数，返回一个 R 型参数。
        //用于将一个类型转换成另外一个类型正合适，这也是 map的初衷所在，用于改变当前元素的类型，
...
//        }
         ...
         * @Collector
         */
        //Collector是怎么一个意思，这是 collect的另一个重载方法，可以理解为它的参数是按顺序执行的，
        //这样就清楚了，这就是个 ArrayList 从创建到调用 addAll方法的一个过程。
        //
//        private static void collect(){
//            Stream<Integer> integerStream = Stream.of(1,2,5,7,8,12,33);
...
//                    ArrayList::addAll);
//        }
        //我们在自定义 Collector的时候其实也是这个逻辑，不过我们根本不用自定义，
        //Collectors已经为我们提供了很多拿来即用的收集器。
        //比如我们经常用到Collectors.toList()、Collectors.toSet()、Collectors.toMap()。
        //另外还有比如Collectors.groupingBy()用来分组，比如下面这个例子，按照 userId 字段分组，返回以 userId 为key，List 为value 的 Map，或者返回每个 key 的个数。
        //
        //              //返回 userId:List<User>
         ...
         *  @reduce
         */
        //它的作用是每次计算的时候都用到上一次的计算结果，比如求和操作，前两个数的和加上第三个数的和，
         ...
         * @并行 Stream
         */
        //Stream 本质上来说就是用来做数据处理的，为了加快处理速度，Stream API 提供了并行处理 Stream 的方式。
        //通过 users.parallelStream()或者users.stream().parallel() 的方式来创建并行 Stream 对象，支持的 API 和普通 Stream 几乎是一致的。
        //
    ...
    public static class FunctionTest {//例子原来没有static
